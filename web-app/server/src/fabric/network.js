...
exports.connectToNetwork = async function (userName) {
  ...
  try {
    ...
    if (!userExists) {
      ...
      let response = {};
      response.error = 'Não foi possível encontrar registro para o usuário ' + userName + '. Solicite o cadastramento ao mesário.';
      return response;
    }
    ...
    let response = {};
    response.error = 'Erro ao processar transação';
    return response;
  } finally {
...
exports.registerVoter = async function (voterId, registrarId, firstName, lastName) {
  ...
  if (!registrarId || !voterId || !firstName || !lastName) {
    let response = {};
    response.error = 'Você precisa preencher todos os campos antes de se registrar!';
    return response;
  }
  ...
  try {
    ...
    if (userExists) {
      ...
      response.error = `Erro! Já existe uma identidade para o usuário ${voterId} na carteira. Por favor, insira
       um número de licença diferente.`;
      return response;
    }
    ...
    if (!adminExists) {
      ...
      let response = {};
      response.error = `Não existe uma identidade para o usuário administrador ${appAdmin} na carteira. 
        Execute o aplicativo enrollAdmin.js antes de tentar novamente`;
      return response;
    }
    ...
    let response = `Eleitor ${firstName} ${lastName} registrado com sucesso. Use sua identificação de eleitor para realizar o login.`;
    return response;
  } catch (error) {
    console.error(`Failed to register user + ${voterId} + : ${error}`);
    let response = {};
    response.error = 'Falha ao registrar o usuário';
    return response;
  }
};