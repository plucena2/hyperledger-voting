...
class MyAssetContract extends Contract {
  ...
  async init(ctx) {
    ...
    let repVotable = await new VotableItem(ctx, '91', ballotData.fedDemocratBrief);
    let demVotable = await new VotableItem(ctx, '92', ballotData.republicanBrief);
    let indVotable = await new VotableItem(ctx, '93', ballotData.greenBrief);
    let grnVotable = await new VotableItem(ctx, '94', ballotData.independentBrief);
    let libVotable = await new VotableItem(ctx, '95', ballotData.libertarianBrief);
  ...
  async createVoter(ctx, args) {
    ...
    if (currElections.length === 0) {
      let response = {};
      response.error = 'Sem eleições. Execute a função init () primeiro';
      return response;
    }
    ...
    let response = `Eleitor ${newVoter.voterId} atualizado`;
    return response;
  }
  ...
  async deleteMyAsset(ctx, myAssetId) {
    ...
    if (!exists) {
      ...
      throw new Error(`A pesquisa para ${myAssetId} não existe`);
    }
  ...
  async readMyAsset(ctx, myAssetId) {
    ...
    if (!exists) {
      ...
      response.error = `A pesquisa para ${myAssetId} não existe`;
      return response;
    }
  ...
  async castVote(ctx, args) {
    ...
    if (electionExists) {
      ...
      if (voter.ballotCast) {
        let response = {};
        response.error = 'O eleitor já realizou sua votação';
        return response;
      }
      ...
      if (parsedCurrentTime >= electionStart && parsedCurrentTime < electionEnd) {
        ...
        if (!votableExists) {
          let response = {};
          response.error = 'O ID do candidato não existe';
          return response;
        }
        ...
        let response = {};
        response.error = 'A eleição não está aberta agora';
        return response;
      }

    } else {
      let response = {};
      response.error = 'A eleição ou o eleitor não existe';
      return response;
    }
  }
